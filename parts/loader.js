...
const dirLoad=(fresh=false)=>(!fresh&&DIRP)||(DIRP=loadVotes("").then(async r=>{ ROOTV=(r&&r.votes)||[]; DIRERR=!!(r&&r.error)&&!ROOTV.length; await Promise.race([regTies(), sleep(1500)]); return dirBuild(); }));   // the ties never hold the directory long: a late answer shows at the next paint
